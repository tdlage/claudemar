...
```
/home/claudemar/claudemar/projects/claudemar/claudemar/
...
The file `/home/claudemar/claudemar/projects/claudemar/claudemar/orchestrator/agents.md` is automatically maintained and contains a brief description of what each agent does, their expertise, and when to delegate tasks to them. **Always read this file before deciding how to handle a task** — if an agent is better suited for the request, delegate to them via the messaging system.
...
### Create an agent
```bash
mkdir -p /home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/{context,inbox,outbox,output,schedules}
```
Then create `/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/CLAUDE.md` with the agent's persona, role, and instructions.

Agent names must match: `/^[a-zA-Z0-9._-]+$/`

### Remove an agent
```bash
rm -rf /home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>
```
Also remove related schedules from crontab and `schedules.json`.

### Edit agent instructions
Edit `/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/CLAUDE.md` directly.

### Add context to an agent
Place markdown files in `/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/context/`.
...
### Create a project
```bash
mkdir -p /home/claudemar/claudemar/projects/claudemar/claudemar/projects/<name>
```
Project names must match: `/^[a-zA-Z0-9._-]+$/`

### Clone a repository into a project
```bash
cd /home/claudemar/claudemar/projects/claudemar/claudemar/projects/<project> && git clone <url> [name]
```
Each project can contain multiple repositories (e.g. frontend, backend, infra).

### Remove a project
```bash
rm -rf /home/claudemar/claudemar/projects/claudemar/claudemar/projects/<name>
```

### Remove a repository from a project
```bash
rm -rf /home/claudemar/claudemar/projects/claudemar/claudemar/projects/<project>/<repo>
```
...
```
/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/inbox/DE-orchestrator_<timestamp>_broadcast.md
```

### Check an agent's inbox
Read files in `/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<name>/inbox/`

## Scheduling

Schedule metadata is stored in `/home/claudemar/claudemar/projects/claudemar/claudemar/schedules.json`:
```json
[{
  "id": "8-char-uuid",
  "agent": "agent-name",
...
2. Create executable bash script at `/home/claudemar/claudemar/projects/claudemar/claudemar/agents/<agent>/schedules/<slug>-<id>.sh`
...
### List schedules
Read `/home/claudemar/claudemar/projects/claudemar/claudemar/schedules.json`
...
File: `/home/claudemar/claudemar/projects/claudemar/claudemar/.env`

Key variables you may need to adjust:
- `DASHBOARD_PORT` — dashboard web port (current: 3000)
- `MAX_OUTPUT_LENGTH` — max Telegram message length (current: 4096)

Do NOT modify `TELEGRAM_BOT_TOKEN` or `ALLOWED_CHAT_ID` unless explicitly asked.

## Email

If email is configured (`.email-credentials` exists in `/home/claudemar/claudemar/projects/claudemar/claudemar`), you and agents can send emails:

```bash
sudo /home/claudemar/claudemar/projects/claudemar/claudemar/send-email.sh --to "recipient@email.com" --subject "Subject" --body "Content"
```
...
Without `--from`, uses the default sender configured in Settings.
...
Admin email for system notifications, configurable in dashboard Settings.

## Execution History

File: `/home/claudemar/claudemar/projects/claudemar/claudemar/history.jsonl` (one JSON object per line, most recent at end)
