...
export function ModelSelector({ models, value, disabled, onChange }: {
  ...
  return (
    <select aria-label="Modelo" title="Modelo e provider usados nesta conversa" value={value} disabled={disabled || !models.length}
      ...
      className="text-xs bg-surface border border-border rounded-md px-2 py-1 text-text-primary max-w-64 disabled:opacity-50">
      ...
      {providers.map(([id, label]) => <optgroup key={id} label={label}>
        {models.filter((model) => model.providerId === id).map((model) => <option key={model.model} value={model.model}>{model.displayName} · {model.providerLabel}</option>)}
      </optgroup>)}
    </select>
  );
}
