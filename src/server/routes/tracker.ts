...
function canAccessBetProject(req: Express.Request, projectName: string): boolean {
  const ctx = req.ctx!;
  if (ctx.role === "admin") return true;
  if (!projectName) return true;
  return ctx.projects.includes(projectName);
}

// ── Cycles ──

trackerRouter.get("/cycles", async (_req, res) => {
  const cycles = await trackerManager.getCycles();
  res.json(cycles);
});

trackerRouter.post("/cycles", requireAdmin, async (req, res) => {
  const { name, startDate, endDate, cooldownEndDate } = req.body;
  if (!name || !startDate || !endDate || !cooldownEndDate) {
    res.status(400).json({ error: "name, startDate, endDate, cooldownEndDate required" });
    return;
  }
  const author = getAuthor(req);
  const cycle = await trackerManager.createCycle({ name, startDate, endDate, cooldownEndDate, createdBy: author.id });
...
trackerRouter.get("/cycles/:cycleId/bets", async (req, res) => {
  let bets = await trackerManager.getBetsByCycle(req.params.cycleId);
  if (req.ctx!.role === "user") {
    const projects = req.ctx!.projects;
    bets = bets.filter((b) => !b.projectName || projects.includes(b.projectName));
  }
  res.json(bets);
});

trackerRouter.post("/bets", requireAdmin, async (req, res) => {
  const { cycleId, title, description, appetite, projectName, assignees, tags } = req.body;
  if (!cycleId || !title) {
    res.status(400).json({ error: "cycleId and title required" });
    return;
  }
  const author = getAuthor(req);
  const bet = await trackerManager.createBet({ cycleId, title, description, appetite, projectName, assignees, tags, createdBy: author.id });
...
trackerRouter.put("/bets/:id", async (req, res) => {
  ...
  if (!canAccessBetProject(req, bet.projectName)) {
...
trackerRouter.patch("/bets/:id/move", async (req, res) => {
  ...
  const bet = await trackerManager.moveBet(req.params.id, status, position ?? 0);
  if (!bet) { res.status(404).json({ error: "Bet not found" }); return; }
  res.json(bet);
});
...
trackerRouter.get("/bets/:betId/scopes", async (req, res) => {
  const scopes = await trackerManager.getScopesByBet(req.params.betId);
  res.json(scopes);
});

trackerRouter.post("/scopes", async (req, res) => {
  const { betId, title, description, assignees } = req.body;
  if (!betId || !title) {
    res.status(400).json({ error: "betId and title required" });
    return;
  }
  const scope = await trackerManager.createScope({ betId, title, description, assignees });
  res.status(201).json(scope);
});

trackerRouter.put("/scopes/:id", async (req, res) => {
  const scope = await trackerManager.updateScope(req.params.id, req.body);
  if (!scope) { res.status(404).json({ error: "Scope not found" }); return; }
  res.json(scope);
});

trackerRouter.patch("/scopes/:id/hill", async (req, res) => {
  const { hillPosition } = req.body;
  if (hillPosition === undefined) { res.status(400).json({ error: "hillPosition required" }); return; }
  const scope = await trackerManager.updateHillPosition(req.params.id, hillPosition);
  if (!scope) { res.status(404).json({ error: "Scope not found" }); return; }
  res.json(scope);
});

trackerRouter.delete("/scopes/:id", async (req, res) => {
  const deleted = await trackerManager.deleteScope(req.params.id);
  if (!deleted) { res.status(404).json({ error: "Scope not found" }); return; }
  res.json({ deleted: true });
});
...
trackerRouter.get("/comments/:targetType/:targetId", async (req, res) => {
  ...
  if (targetType !== "bet" && targetType !== "scope") {
    res.status(400).json({ error: "targetType must be bet or scope" });
    return;
  }
...
trackerRouter.post("/comments", async (req, res) => {
  ...
  const comment = await trackerManager.addComment({
    targetType, targetId, authorId: author.id, authorName: author.name,
    content, attachments,
  });
...
trackerRouter.post("/commit-links", async (req, res) => {
  const { scopeId, projectName, repoName, commitHash, commitMessage } = req.body;
  if (!scopeId || !projectName || !repoName || !commitHash) {
    res.status(400).json({ error: "scopeId, projectName, repoName, commitHash required" });
    return;
  }
  const author = getAuthor(req);
  const link = await trackerManager.linkCommit({ scopeId, projectName, repoName, commitHash, commitMessage, linkedBy: author.id });
  res.status(201).json(link);
});

trackerRouter.delete("/commit-links/:id", async (req, res) => {
  const deleted = await trackerManager.unlinkCommit(req.params.id);
  if (!deleted) { res.status(404).json({ error: "Link not found" }); return; }
  res.json({ deleted: true });
});

trackerRouter.get("/scopes/:id/commits", async (req, res) => {
  const commits = await trackerManager.getCommitsByScope(req.params.id);
  res.json(commits);
});
...
trackerRouter.post("/test-runs", async (req, res) => {
  ...
  if (!testCaseId || !status) {
    res.status(400).json({ error: "testCaseId and status required" });
    return;
  }
  const author = getAuthor(req);
...
trackerRouter.post("/test-run-comments", async (req, res) => {
  ...
  if (!testRunId || !content) {
    res.status(400).json({ error: "testRunId and content required" });
    return;
  }
  const author = getAuthor(req);
